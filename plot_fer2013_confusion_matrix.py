...
def plot_confusion_matrix(cm, classes,
    ...
    ytick_marks = np.arange(-0.5,len(classes))
...
# checkpoint = torch.load(os.path.join(path,'model_saved2_better',opt.split + '_model.t7'))
...
plot_confusion_matrix(matrix, classes=class_names, normalize=True,
                      title= opt.split+' Confusion Matrix (Accuracy: %.3f%%)' %acc)
